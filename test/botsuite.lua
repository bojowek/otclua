...
-- ============================================================================
S('Exchange money converts one configured currency stack')
do
    local F = newWorld({ '.....', '..@..', '.....' })
    local bag = addContainer(F.st, 1, ID_BACKPACK, {
        { id = ID_GOLD, count = 100 },
        { id = 3035, count = 100 },
    })
    local H = newHost(F)
    local exchange = H.bot.modules.money
    H.bot.storage.moneyItems = { { id = ID_GOLD, count = 1 } }
    exchange:setOn()
    H.sender:clear()
    exchange:tick()

    local uses = H.sender:byKind('use')
    eq(#uses, 1, 'one conversion is sent per tick')
    eq(uses[1].id, ID_GOLD, 'table-form money item is matched')
    eq(uses[1].stack, 0, 'the first container slot is addressed')
    eq(uses[1].index, 0, 'the native use path gets a free container id')
    eq(uses[1].pos.y, 0x40 + bag.id, 'the container id is encoded in the position')

    H.bot.storage.moneyItems = { 3035 }
    H.sender:clear()
    exchange:tick()
    eq(#H.sender:byKind('use'), 1, 'numeric-form money item is matched')
    eq(H.sender:byKind('use')[1].id, 3035, 'numeric currency id is used')

    bag.items[2].count = 99
    local lootBag = addContainer(F.st, 2, ID_CORPSE, {
        { id = 3035, count = 100 },
    })
    lootBag.lootContainer = true
    H.sender:clear()
    exchange:tick()
    eq(#H.sender:byKind('use'), 0, 'non-100 and loot-container stacks are ignored')
end
...
do
    ...
    local offers = {}
    for i = 1, 20 do offers[i] = { id = 3000 + i, sellPrice = 1, subType = 0 } end
    F.st.npcTrade = { open = true, npcName = 'Rashid', items = offers }
    local list = {}
...
S('REVIEW: sellall ignores equipped items and batches container stacks')
do
    local F = newWorld({ '@.' })
    local H = newHost(F)
    local cb = H.cb
    local npcPos = F.at(1, 0)
    F.st:addCreature({ id = 5557, name = 'Rashid', type = 2, isNpc = true, pos = npcPos })
    F.st.npcTrade = {
        open = true,
        npcName = 'Rashid',
        items = {
            { id = 3436, subType = 7, sellPrice = 100, buyPrice = 0 },
            { id = 8063, subType = 0, sellPrice = 100, buyPrice = 0 },
        },
    }
    F.st.player.inventory[4] = { kind = 'item', id = 8063, count = 1 }
    local loot = addContainer(F.st, 0, ID_BACKPACK, {
        { kind = 'item', id = 3436, count = 2 },
        { kind = 'item', id = 3436, count = 3 },
    })

    H.sender:clear()
    eq(cb:_actionSellAll('Rashid', 0), 'retry', 'container item starts a sell request')
    local request = H.sender:byKind('sell')[1]
    ok(request ~= nil, 'container item produced a sell packet')
    eq(request.id, 3436, 'container item id is sold')
    eq(request.sub, 7, 'NPC offer subtype is used')
    eq(request.amount, 5, 'matching container stacks are batched')

    loot.items = {}
    eq(cb:_actionSellAll('Rashid', 1), true, 'confirmed container sale completes sellall')
end

S('REVIEW: sellall uses PlayerGoods for paginated containers')
do
    local F = newWorld({ '@.' })
    local H = newHost(F)
    local cb = H.cb
    F.st:addCreature({ id = 5558, name = 'Rashid', type = 2, isNpc = true, pos = F.at(1, 0) })
    F.st.npcTrade = {
        open = true,
        npcName = 'Rashid',
        items = {
            { id = 3436, subType = 0, sellPrice = 100, buyPrice = 0 },
            { id = 8063, subType = 0, sellPrice = 100, buyPrice = 0 },
        },
        playerGoods = { [3436] = 1, [8063] = 1 },
    }
    F.st.player.inventory[4] = { kind = 'item', id = 8063, count = 1 }

    H.sender:clear()
    eq(cb:_actionSellAll('Rashid', 0), 'retry', 'PlayerGoods starts a sell request')
    local request = H.sender:byKind('sell')[1]
    ok(request ~= nil, 'paginated-container item produced a sell packet')
    eq(request.id, 3436, 'PlayerGoods item id is sold')
    eq(request.amount, 1, 'PlayerGoods quantity is used')
end
